...
gulp.task('js:babel', function() {
    return gulp.src(requireJsOptimizerConfig.baseUrl + '/**')
        .pipe(es.map(function(data, cb) {
            if (!data.isNull()) {
                babelTranspile(slash(data.relative), function(err, res) {
                    if (res) {
...
gulp.task('css', function () {
    var bowerCss = gulp.src('src/bower_modules/components-bootstrap/css/bootstrap.min.css')
        ...
        fontAwesome= gulp.src('src/bower_modules/font-awesome/css/font-awesome.min.css')
                .pipe(replace(/url\((')?\.\.\/fonts\//g, 'url($1fonts/')),
        toatr = gulp.src('src/bower_modules/toastr/toastr.min.css'),
        combinedCss = es.concat(bowerCss, appCss, fontAwesome, toatr).pipe(concat('css.css')),
        ...
        middleware: function(connect, opt) {
            return [
                 function (req, res, next) {                     
                     ...
                     babelTranspile(slash(pathname), function(err, result) {
